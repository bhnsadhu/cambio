...
`NotificationProvider` owns one notification viewport. `NotificationArea` centers it in the original space between the player hands and round controls. Notifications take their natural height: there is no height cap or nested scroll container, and the page grows so players can scroll down to the controls. Desktop dialogs retain the table position while including notification actions in their focus scope.

Use the original Cambio treatments: wide charcoal event bars with caption labels, dividers, and player chips; larger centered headlines for major moments; full-size card reveals and countdowns; and pause requests with their roster and actions. Account and social feedback share the same charcoal surfaces, responsive panel corners, typography, and restrained mint/red cues. The compact toast cards and category icons have been removed. New notification types should use the shared portal while preserving the treatment suited to their content.
